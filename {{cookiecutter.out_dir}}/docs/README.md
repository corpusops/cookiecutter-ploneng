...
## plone settings setup
To add/modify plone settings, you specifically need to edit ``.ansible/playbooks/roles/plone_vars/defaults/main.yml`` for hosted deployments,<br/>
and adapt certainly one of ``cops_plone_docker_env_freeform``, ``cops_plone_docker_env_defaults``, ``cops_plone_docker_env_extra``.
  - Here, never forget to abuse ansible variables to cut variables from their templates<br/>
    ...
    eg:
      - In all cases, adapt/edit inventory (variables may already exist)
        ```sh
        $EDITOR .ansible/playbooks/roles/plone_vars/defaults/main.yml
        ```

          ```yaml
          # ...
          cops_plone_docker_env_extra:
            MYSETTINGS={{'{{'}}cops_plone_foobar}}
            MYSECRETSETTINGS={{'{{'}}cops_plone_footruc}}
          ```
      ...
      - Set crypted values (see ansible/README about vaults) in related crypted group vars
        ...
        ```
          ...
          ```yaml
          # ...
          cops_plone_footruc: abcd123456789secret
          ```
